...
__constant__ static const uint8_t rsbox[256] = {
...
#define getHostSBoxValue(num) (host_sbox[(num)])
...
static void KeyExpansion(uint8_t* RoundKey, const uint8_t* Key) {
    ...
    for (i = Nk; i < Nb * (Nr + 1); ++i) {
        ...
        if (i % Nk == 0) {
            ...
            // Function Subword()
            {
                tempa[0] = getHostSBoxValue(tempa[0]);
                tempa[1] = getHostSBoxValue(tempa[1]);
                tempa[2] = getHostSBoxValue(tempa[2]);
                tempa[3] = getHostSBoxValue(tempa[3]);
            }
...
__device__ static void XorWithIv(uint8_t* buf, const uint8_t* Iv) {
    uint8_t i;
